...
- バリデーションの違い
  - インターフェース層のルーティング定義やクライアントコンポーネントでは、ユーザーにフィードバックを提供するためのバリデーションを行う
    - 提供している正規のクライアントから送信可能なデータのみにユーザーフレンドリーなエラーメッセージを返す
      - 必要に応じて、入力値の種類ごとに細かいフィードバックを行う
    - 正規のクライアントから送信が困難な形式のデータに対しては、工数やセキュリティの観点で詳細な内部情報はフィードバックしない
      - HTMLのフォームバリデーション属性で担保されている場合など

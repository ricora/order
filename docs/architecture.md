...
- `app/`
  ...
  - `routes/`
    - アプリケーションのルーティング定義
    - `-components/`
      - 特定のルート配下でのみ再利用されるコンポーネント
    - `-helpers/`
      - 特定のルート配下でのみ再利用される補助関数
    - `-hooks/`
      - 特定のルート配下でのみ再利用されるフック
  - `usecases/`
    - ユースケースの実装
  - `utils/`
...
- クライアントではHono clientを使用して型安全にAPIと通信する
  - ヘルパー関数`createHonoClient()`を利用する
    - 実体は`app/routes/-helpers/api/hono-client.ts`
    - `AppType`は`app/routes/api/index.tsx`からimportする
...
- インターフェース層
  - 外部から入力手段を定義する
    ...
    - ルート層では`app/routes/_middleware.ts`でDBクライアントをコンテキストに注入（`c.set("dbClient", ...)`）し、この`dbClient`を起点にユースケースを呼び出す
- ユースケース層
